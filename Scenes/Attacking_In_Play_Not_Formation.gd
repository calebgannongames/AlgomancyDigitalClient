...
var screen_location = Vector2(0, 0)
var card_scale = 1.0
...
func position_units():
	var units = get_children()
	var max_width = units.size()*2.2  # The total space to distribute units
	var unit_spacing = 2.2  # Avoid division by zero if only one unit
	#print(unit_spacing)
	for unit in units:
		var index = unit.get_index()
		var offset = index * unit_spacing - (max_width / 2.0) + 1  # Center units around the middle of the max_width
...
func card_not_hovered_anymore(card):
	card.outline.visible = false
	card.remove_trigger_outlines()
	card.delete_trigger_lines()
	#card.trigger_origin_show_curve.hide()
