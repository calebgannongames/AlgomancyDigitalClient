...
var screen_location = Vector2(5, -3)
var card_scale = 1.0
...
func position_units():
	...
	for column in formation:
		...
		for unit in column:
			...
			unit.target_transform.origin = Vector3(xoffset + screen_location[0]/card_scale, table_height, screen_location[1]/card_scale + yoffset)
...
func _on_child_entered_tree(node):
	...
	if node.is_spell_token:
		var target_node = get_node('/root/Main/Player')
		node.call_deferred("move_node", target_node.active_board)
		#node.move_node(target_node.active_board)
	else:
